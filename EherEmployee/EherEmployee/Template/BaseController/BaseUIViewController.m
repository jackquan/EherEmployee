...
fromPresentController:(UIViewController *)fromPresentController {
    MPMoviePlayerViewController *playerViewController =
    [[MPMoviePlayerViewController alloc]
     initWithContentURL:[NSURL fileURLWithPath:fileName]];
    
    [fromPresentController
     presentMoviePlayerViewControllerAnimated:playerViewController];
    MPMoviePlayerController *player = [playerViewController moviePlayer];
    [player play];
    
    tmpProcessPlayerControlViewIndex = 0;
    [self processPlayControlView:playerViewController.view];
}

//插放视频
+ (void)playMovieByRemote:(NSString *)url
    fromPresentController:(UIViewController *)fromPresentController {
    MPMoviePlayerViewController *playerViewController =
    [[MPMoviePlayerViewController alloc]
...
//显示AlertViewCtl
- (void)showAlertViewCtl:(NSString *)title msg:(NSString *)msg cancel:(NSString *)cancel
{
    ...
    UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:cancel style:UIAlertActionStyleCancel handler:nil];
    [alertCtl addAction:confirmAction];
    [self presentViewController:alertCtl animated:YES completion:nil];
}
...
- (void)requestFinish:(BaseRequest *)request dataArr:(NSArray *)dataArr {
    ...
    if (dataArr.count > 0) {
        if ([dataArr.firstObject isKindOfClass:[Version_Modal class]]) {
            Version_Modal *modal = dataArr.firstObject;
            if (modal.restCode &&
                [modal.restCode isEqualToString:Request_NewVersion]) {
                [self showUpdateVersionAlertView:modal];
                return;
            }
        }
    }
...
- (void)showUpdateVersionAlertView:(Version_Modal *)model {

    if (![model isKindOfClass:[Version_Modal class]]) {
        return;
    }
    
    NSString *title = [NSString stringWithFormat:@"检测到新版本%@\n是否更新？",
                       model.versionCode_];
    
    [self showChooseAlertViewCtl:title msg:nil okBtnTitle:@"取消" cancelBtnTitle:@"立即更新" confirmHandle:nil cancleHandle:^{
        if (!model.path_) {
            [BaseUIViewController
             showAlertView:@"下载地址解析异常"
             msg:@"请联系一禾客服"
             cancel:@"知道了"];
        } else {
            [[UIApplication sharedApplication]
             openURL:[NSURL URLWithString:model.path_]];
        }
    }];
