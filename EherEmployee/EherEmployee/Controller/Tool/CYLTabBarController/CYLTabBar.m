...
static CGFloat CYLTabBarBoundsWidth;
...
- (void)layoutSubviews {
    ...
    CGFloat taBarWidth = CYLTabBarBoundsWidth>0.1?CYLTabBarBoundsWidth:self.bounds.size.width;
    CGFloat taBarHeight = self.bounds.size.height;
    CYLTabBarItemWidth = (taBarWidth - CYLPlusButtonWidth) / CYLTabbarItemsCount;
    
    CGFloat offestX = (self.bounds.size.width - taBarWidth)/2;
    ...
    self.plusButton.center = CGPointMake(taBarWidth * 0.5 + offestX, taBarHeight * multiplierOfTabBarHeight + constantOfPlusButtonCenterYOffset);
...
+ (void)setTabBarTotalWidth:(CGFloat)width
{
    CYLTabBarBoundsWidth = width;
}
